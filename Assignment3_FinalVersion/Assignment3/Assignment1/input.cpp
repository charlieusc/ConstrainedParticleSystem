...
void mouseMotionDrag(int x, int y)
{
  ...
  if (g_iRightMouseButton) // handle camera rotations
  {
    ...
    if (g_iLeftMouseButton) {
        ...
        msForceHa.x = vMouseDelta[0]*msforceScale;
        msForceHa.y = vMouseDelta[0]*msforceScale;
        double maxForce = 50;
        if(msForceHa.x > maxForce){
            msForceHa.x = maxForce;
        }else if(msForceHa.x<-maxForce){
            msForceHa.x = -maxForce;
        }
        if(msForceHa.y > maxForce){
            msForceHa.y = maxForce;
        }else if(msForceHa.y<-maxForce){
            msForceHa.y = -maxForce;
        }
        //msForceHa.z = vMouseDelta[0]*msforceScale*0.1;
        /*
        printf("vMouseDelta[0]:%d\t",vMouseDelta[0]);
        
        printf("msFHx:%f\t",msForceHa.x);
        printf("msFHy:%f\t\n",msForceHa.y);
        printf("msFUpx:%f\t",msForceUp.x);
        printf("msFUpx:%f\t\n",msForceUp.y);
         */
